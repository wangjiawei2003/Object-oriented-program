...
{
	static int MX_old = -1, MY_old = -1, MAction_old = 0x0001;
		...
		if (InputRec.EventType == KEY_EVENT) {
			...
			if (InputRec.Event.KeyEvent.bKeyDown) { // 只在按下时判断，弹起时不判断
				...
				switch (InputRec.Event.KeyEvent.wVirtualKeyCode) {
					case VK_UP:
						keycode1 = 0xe0;
						keycode2 = 72;		//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和72（0x48）
						break;
					case VK_DOWN:
						keycode1 = 0xe0;
						keycode2 = 80;	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和80（0x50）
						break;
					case VK_LEFT:
						keycode1 = 0xe0;
						keycode2 = 75;	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和75（0x4B）
						break;
					case VK_RIGHT:
						keycode1 = 0xe0;
						keycode2 = 77;	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和77（0x4D）
						break;
					default:
						break;
				} //end of switch
